...
function renderTimer(){
  ...
  // when time is up, timer will turn black
  if(secondsElapsed >= totalSeconds){
    document.querySelector('.timer').classList.add("state-times-up");
  }
  else{
    document.querySelector('.timer').classList.remove("state-times-up");
  }

  // seconds-hand positioning
  var incrementSec = 360 / 60;
  var positionSec = 360 - incrementSec * (diff % 60);

  document.querySelector('#second-hand').style.transform = "rotate(" + positionSec + "deg)";

  // main-hand positioning
  var incrementSec = 360 / totalSeconds;
  var positionSec = incrementSec * secondsElapsed;

  document.querySelector('#main-hand').style.transform = "rotate(" + positionSec + "deg)";
